...
class MainComponent extends Component{
    ...
    render(){
        const {topBar,Footwear,cartData,orders,checkData}=this.state
        ...
        // let json1={name:cartData.name,Quantity:cartData.Quantity,Amount:cartData.Amount}
        //  checkData=cartData;
        //  console.log(checkData);
        let user=auth.getUser();
        return (
            <div className="container">
            <Navbar  topBar={topBar} Footwear={Footwear} cartItem={cartItem} user={user} orders={orders} />
            <Switch>
            <Route path="/addProducts/:id" render={(props)=><EditProduct {...props}   />} />
            <Route path="/delete/:id" render={(props)=><Delete {...props}   />} />
            <Route path="/addProducts" render={(props)=><AddProduct {...props}   />} />
            <Route path="/manageOrders"  component={ManageProduct}  />
                <Route path="/myOrders"  component={MyOrders}  />
            <Route path="/checkout"  render={(props)=>user ? <Checkout {...props} cartData={cartData} cartItem={cartItem} account={account}  /> : <Redirect to="/login" />} />
            <Route path="/logout"  component={Logout} />
              <Route path="/login"  component={Login} />
            <Route path="/products" render={(props)=><MainFile {...props}  handleCart={this.handleCartData} cartData={cartData} />} />
            <Route path="/cart" render={(props)=><Cart {...props} cartData={cartData} cartItem={cartItem} account={account}  onhandle={this.handleAdd} onhandleMinus={this.handleMinus} />} />
